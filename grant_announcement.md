...
*   **Grant Specific Details:**
    *   `{grant_title}`: The official title of the grant or funding opportunity (e.g., "Community Impact Grant 2025").
    *   `{grant_introduction}`: A brief paragraph introducing the grant and its overall purpose.
    *   `{grant_objectives_list}`: A list of specific objectives this grant aims to achieve through funded projects.
    *   `{grant_eligibility_criteria_text}`: Detailed text describing who is eligible to apply (e.g., types of organizations, geographic restrictions beyond the general focus, specific requirements).
    *   `{grant_funding_amount_details_text}`: Information about the funding available (e.g., "Grants will range from INR 200,000 to INR 500,000," or "Total available fund: INR 2,000,000").
    *   `{grant_project_duration_text}`: Expected duration of projects to be funded (e.g., "6 to 12 months").
    *   `{grant_application_deadline_date}`: The closing date for applications.
    *   `{grant_application_procedure_text}`: Step-by-step instructions on how to apply (e.g., "Download application form from [link], submit to [email address]").
    *   `{grant_selection_process_text}`: (Optional) Brief overview of how applications will be reviewed and selected.
    *   `{grant_reporting_requirements_text}`: (Optional) Brief mention of reporting expectations for successful grantees.
...
1.  **Structure the Announcement:** Organize the Markdown document logically with clear headings. A suggested structure:
    ...
    *   `## Focus Areas`
        *   `### Thematic Areas` (listing from `{grant_thematic_areas_list}`)
...
## Introduction
{grant_introduction} (Example: We are pleased to announce the launch of the Community Empowerment Grant 2025, aimed at supporting grassroots initiatives that foster sustainable development and social change.)
...
*   Area 2 from {grant_thematic_areas_list}
*   ...

### Geographic Focus
{grant_geographic_focus_summary} (Example: This grant prioritizes projects implemented in the states of Bihar and Jharkhand, with special consideration for initiatives in rural districts.)

## Eligibility Criteria
{grant_eligibility_criteria_text} (Example: Eligible applicants include non-profit organizations registered in India, community-based organizations, and social enterprises with a clear charitable purpose. Applicants must have been operational for at least two years...)
...
**Output:** Generate *only* the Markdown content for the Grant Announcement based on the provided input variables.
